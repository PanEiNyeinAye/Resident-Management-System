        ...
        <div class="content">
            ...
            <div class="card">
            				<div class="card-header" style="text-align:right; background-color:#3A4D6F; color:#fff;">
                            	<div style="text-align:center;position:relative;"><Strong>Criminal Records</Strong></div>
         						<a href="/ResidentManagementSystem/policeOfficer/getAddCriminalRecord/${policeOfficer.policeOfficerId}" style="color:#fff"><i class="material-icons" style="color:#fff">&#xE147;</i> Add New Record</a>
                            </div>
                            <div class="card-body">
            <table id="bootstrap-data-table" class="table table-striped table-bordered">
                <thead>
                    <tr>
						<th>No.</th>
                        <th>Criminal Section</th>
						<th>Date of Crime</th>
						<th>Police Officer's Sign</th>
						<th>Place of Crime Scene</th>
						<th>The offender's NRC No</th>
						<th>The Result of The Trial</th>
						<th>Edit</th>
                    </tr>
                </thead>
                <tbody>
                 <c:forEach var="crime" items="${crimeList}" varStatus="crimeCount">
                    <tr>
                    	<td><c:out value="${crimeCount.count}"/></td>
                    	<td><c:out value ="${crime.crimeName}"/></td>
						<td><c:out value ="${crime.criminalRecordDate}"/></td>
						<td><c:out value ="${crime.policeOfficerSign}"/></td>
						<td><c:out value ="${crime.crimeScenePlace}"/></td>
						<td><c:out value ="${crime.resident.nrcNo}"/></td>
						
						<c:if test="${crime.resultOfTheTrial eq 1}"><td><c:out value ="Still Facing The Trial"/></td></c:if>
						<c:if test="${crime.resultOfTheTrial eq 2}"><td><c:out value ="Losed The Trial"/></td></c:if>
						<c:if test="${crime.resultOfTheTrial eq 3}"><td><c:out value ="Won The Trial"/></td></c:if>
						<td>
                            <a href="/ResidentManagementSystem/policeOfficer/getEditCriminalRecord/${crime.crimeId}/${policeOfficer.policeOfficerId}" class="fa fa-pencil-square-o" aria-hidden="true"></a>
                        </td>				
					</tr>	
					 </c:forEach>				
                </tbody>
